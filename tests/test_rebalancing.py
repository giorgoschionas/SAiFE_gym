...
def lu_action(lower, upper):
    """Build a 2-element action in (center_offset, half_width) format from
    a desired (lower, upper) tick range.

    Allows tests to be written naturally in (lower, upper) space; the
    action space the env consumes is (center, half_width).
    """
    center = (lower + upper) / 2.0
    half_width = (upper - lower) / 2.0
    return np.array([[center, half_width]], dtype=np.float64)
...
class TestPoolLiquidityModification:
    ...
    def test_full_array_after_subsequent_rebalance(self):
        ...
        model.update_state(arrivals, lu_action(-3, 2))

        # Second rebalance: move to [0, 4)
        model.update_state(arrivals, lu_action(0, 4))
...
class TestActionValidation:
    """Test action handling under the (center_offset, half_width, hold_flag)
    parameterization. Verified end-to-end via update_state, since validate_action
    no longer exists as a separate method."""

    def test_action_space_shape(self):
        """Action space should be 3-element (center_offset, half_width, hold_flag)."""
        model = create_test_model(tau=5)
        space = model.get_action_space()
        assert space.shape == (3,)

    def test_action_space_bounds(self):
        """Box bounds: center ∈ [-tau, tau], half_width ∈ [1, tau], hold_flag ∈ [-1, 1]."""
        tau = 5
        model = create_test_model(tau=tau)
        space = model.get_action_space()
        assert np.allclose(space.low, [-tau, 1.0, -1.0])
        assert np.allclose(space.high, [tau, tau, 1.0])

    def test_action_resolves_to_correct_range(self):
        """(center=5, half_width=3) → LP at (current+2, current+8)."""
        model = create_test_model(tau=10, num_ticks=200)
        initialize_state(model, liquidity_value=1e6)
        current_tick = int(model.state[POOL_CURRENT_TICK_KEY][0])
        ...
        assert model.state[LP_TICK_UPPER_KEY][0] == current_tick + 8

    def test_half_integer_center_and_width(self):
        """Round at (lower, upper), not at (center, half_width) — so half-integer
        centers/widths still snap to ticks correctly. (center=3.5, hw=1.5) → (2, 5)."""
        model = create_test_model(tau=10, num_ticks=200)
        initialize_state(model, liquidity_value=1e6)
        current_tick = int(model.state[POOL_CURRENT_TICK_KEY][0])

        action = np.array([[3.5, 1.5]], dtype=np.float64)
        arrivals = np.array([[0, 0]], dtype=np.int64)
        model.update_state(arrivals, action)

        assert model.state[LP_TICK_LOWER_KEY][0] == current_tick + 2
        assert model.state[LP_TICK_UPPER_KEY][0] == current_tick + 5

    def test_minimum_one_tick_width(self):
        """Defensive floor: half_width below 0.5 still produces a 1-tick range."""
        model = create_test_model(tau=5)
        initialize_state(model, liquidity_value=1e6)
        current_tick = int(model.state[POOL_CURRENT_TICK_KEY][0])

        # half_width=0.4 rounds (0-0.4)=0 and (0+0.4)=0 — both round to current tick;
        # the np.maximum guard bumps upper to lower+1.
        action = np.array([[0.0, 0.4]], dtype=np.float64)
        arrivals = np.array([[0, 0]], dtype=np.int64)
        model.update_state(arrivals, action)

        assert (model.state[LP_TICK_UPPER_KEY][0]
                - model.state[LP_TICK_LOWER_KEY][0]) >= 1
...
class TestVectorizedRebalance:
    ...
    def test_different_actions_per_trajectory(self):
        ...
        # Different ranges for each trajectory, expressed as (center, half_width).
        action = np.array([
            [0.0,  2.0],   # (lower=-2, upper=2)  centered
            [1.5,  2.5],   # (lower=-1, upper=4)  shifted right
            [-2.5, 1.5],   # (lower=-4, upper=-1) shifted left
        ], dtype=np.float64)
...
class TestHoldAction:
    ...
    def test_hold_preserves_position(self):
        ...
        action_deploy = np.array([[0, 2, -1.0]], dtype=np.float64)
        arrivals = np.array([[0, 0]], dtype=np.int64)
        ...
        action_hold = np.array([[0, 2, 1.0]], dtype=np.float64)
    ...
    def test_mixed_hold_and_rebalance(self):
        ...
        # Deploy both
        action_deploy = np.array([[0, 2, -1.0], [0, 2, -1.0]], dtype=np.float64)
        arrivals = np.zeros((num_traj, 2), dtype=np.int64)
        model.update_state(arrivals, action_deploy)
        ...
        # Traj 0 holds at (lower=-2, upper=2) → center=0, hw=2.
        # Traj 1 rebalances to (lower=-1, upper=3) → center=1, hw=2.
        action_mixed = np.array([[0, 2, 1.0], [1, 2, -1.0]], dtype=np.float64)
    ...
    def test_hold_flag_boundary(self):
        ...
        action = np.array([[0, 2, 0.0]], dtype=np.float64)
        arrivals = np.array([[0, 0]], dtype=np.int64)
        model.update_state(arrivals, action)
