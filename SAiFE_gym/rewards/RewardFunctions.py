...
def _token0_amount(state: dict, exponential_value: float) -> np.ndarray:
    ...
    x_above = np.zeros_like(lp_liq)

    x = np.where(above, x_above, np.where(below, x_below, x_in))
...
class RunningInventoryPenalty(RewardFunction):
    ...
    Following mbt_gym's RunningInventoryPenalty / CjCriterion pattern.
    """
    ...
    def __init__(
        ...
        self.pnl = PnL(exponential_value, initial_wealth)
        self.exponential_value = exponential_value
    ...
    def calculate(
        ...
        return (
            ...
            - dt * self.per_step_inventory_aversion * inventory ** self.inventory_exponent
            - self.terminal_inventory_aversion
            * int(is_terminal_step)
            * inventory ** self.inventory_exponent
        )
